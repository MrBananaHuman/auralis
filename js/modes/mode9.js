...
export function mode9_render(container, currentKey = 'C') {
    ...
    let currentStringSet = [0, 1, 2, 3, 4]; // 기본 1~5번줄 (6번줄 제외)
        ...
        <div class="glass-panel chord-explorer">
            ...
            <div style="display: flex; justify-content: center; align-items: center; gap: 2rem; flex-wrap: wrap; margin-bottom: 1.5rem;">
                ...
                <div style="display: flex; align-items: center; gap: 0.6rem;">
                    <span style="color: var(--text-muted); font-size: 0.7rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px;">Strings</span>
                    ${[1, 2, 3, 4, 5, 6].map(s => `
                        <div style="display: flex; align-items: center; gap: 0.3rem;">
                            <input type="checkbox" id="voicing-string-${s}" class="string-checkbox voicing-string-checkbox" value="${s - 1}" ${currentStringSet.includes(s - 1) ? 'checked' : ''}>
                            <label for="voicing-string-${s}" style="font-size: 0.8rem; font-weight: 600; cursor: pointer; color: var(--text-main);">${s}</label>
                        </div>
                    `).join('')}
                </div>
            ...
            if (effectiveMode === 'all') {
                noteNames.forEach((name, toneIdx) => {
                    ...
                    boardDiv.querySelectorAll(`.fret[data-note="${name}"]`).forEach(cell => {
                        ...
                        if (!currentStringSet.includes(strIdx)) return;
    ...
    const stringCheckboxes = container.querySelectorAll('.voicing-string-checkbox');
    stringCheckboxes.forEach(cb => {
        cb.addEventListener('change', () => {
            currentStringSet = Array.from(stringCheckboxes)
                .filter(c => c.checked)
