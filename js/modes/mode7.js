...
export function mode7_render(container, currentKey) {
    container.innerHTML = `
        <div class="mode-container builder-mode">
            ...
            <!-- Timeline Controls (BPM, Rhythm, Time Signature, Rhythm Pattern Notation) -->
            <div class="timeline-controls card glass" style="margin-bottom: 1rem; padding: 1rem; display: flex; align-items: center; justify-content: space-between; gap: 1.5rem; flex-wrap: wrap; border-radius: 12px; border: 1px solid rgba(255, 255, 255, 0.05); background: rgba(15, 23, 42, 0.4);">
                ...
                <!-- Dynamic Rhythm Sheet SVG -->
                <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 0.25rem;">
                    <label style="color: var(--text-muted); font-size: 0.7rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px;">Rhythm Pattern</label>
                    <div style="background: white; border-radius: 8px; padding: 0.4rem 0.8rem; display: flex; align-items: center; justify-content: center; border: 1px solid rgba(255,255,255,0.1);">
                        <svg id="rhythm-notation-svg" width="160" height="40" viewBox="0 0 160 40" style="background: transparent;">
    ...
    // Helper: Draws visual rhythm slash notation on the staff SVG
    const drawRhythmNotation = () => {
        const svg = container.querySelector('#rhythm-notation-svg');
        if (!svg) return;
        svg.innerHTML = '';
        ...
        line.setAttribute("x1", "5"); line.setAttribute("y1", "20");
        line.setAttribute("x2", "155"); line.setAttribute("y2", "20");
        line.setAttribute("stroke", "#333");
        line.setAttribute("stroke-width", "1.5");
        svg.appendChild(line);

        // 2. Time Signature numbers
        const timeSigText = document.createElementNS("http://www.w3.org/2000/svg", "text");
        timeSigText.setAttribute("x", "10");
        timeSigText.setAttribute("y", "15");
        timeSigText.setAttribute("fill", "#111");
        timeSigText.setAttribute("font-size", "14");
        timeSigText.setAttribute("font-family", "Outfit, sans-serif");
        timeSigText.setAttribute("font-weight", "800");
        timeSigText.innerHTML = `<tspan x="10" dy="2">${timeSig.split('/')[0]}</tspan><tspan x="10" dy="12">${timeSig.split('/')[1]}</tspan>`;
        svg.appendChild(timeSigText);

        // 3. Populate notes
        let numNotes = 1;
        if (rhythm === '4n') numNotes = beats;
        else if (rhythm === '8n') numNotes = beats * 2;
        else if (rhythm === '2n') numNotes = timeSig === '3/4' ? 1 : 2;
        else if (rhythm === '1n') numNotes = 1;

        const startX = 35;
        const endX = 145;
        const width = endX - startX;
        const step = numNotes > 1 ? width / (numNotes - 1) : width / 2;

        for (let i = 0; i < numNotes; i++) {
            const x = numNotes > 1 ? startX + i * step : startX + (endX - startX) / 2;
            const y = 20;

            // Slash note head shape
            const path = document.createElementNS("http://www.w3.org/2000/svg", "path");
            path.setAttribute("d", `M ${x-4} ${y+4} L ${x+4} ${y-4} L ${x+2} ${y-5} L ${x-6} ${y+3} Z`);
            ...
            if (rhythm !== '1n') {
                ...
                stem.setAttribute("x1", x + 3); stem.setAttribute("y1", y - 4);
                stem.setAttribute("x2", x + 3); stem.setAttribute("y2", y - 16);
                stem.setAttribute("stroke", "#4f46e5");
                ...
                if (rhythm === '8n') {
                    ...
                    flag.setAttribute("d", `M ${x+3} ${y-16} Q ${x+8} ${y-12} ${x+7} ${y-6}`);
                    flag.setAttribute("stroke", "#4f46e5");
                    flag.setAttribute("stroke-width", "1.5");
                    flag.setAttribute("fill", "none");
                    svg.appendChild(flag);
                }
            }
            
            // Rest representation for 2n in 3/4 time
            if (rhythm === '2n' && timeSig === '3/4' && i === 0) {
                const restX = x + step;
                const restText = document.createElementNS("http://www.w3.org/2000/svg", "text");
                restText.setAttribute("x", restX);
                restText.setAttribute("y", "23");
                restText.setAttribute("fill", "#666");
                restText.setAttribute("font-size", "18");
                restText.setAttribute("text-anchor", "middle");
                restText.textContent = "𝄾"; // Quarter rest unicode
        ...
        barLine.setAttribute("x1", "155"); barLine.setAttribute("y1", "10");
        barLine.setAttribute("x2", "155"); barLine.setAttribute("y2", "30");
        barLine.setAttribute("stroke", "#333");
        barLine.setAttribute("stroke-width", "2.5");
        svg.appendChild(barLine);
    };

    rhythmSelect.addEventListener('change', drawRhythmNotation);
    timeSigSelect.addEventListener('change', drawRhythmNotation);
    ...
    const renderProgression = () => {
        ...
        progression.forEach((chord, index) => {
            ...
            card.querySelector('.play-card-btn').addEventListener('click', async (e) => {
                ...
                // Play individual card with the selected rhythm strum pattern
                const bpm = parseInt(bpmSlider.value, 10);
                const rhythm = rhythmSelect.value;
                const timeSig = timeSigSelect.value;
                const beatsPerMeasure = timeSig === '3/4' ? 3 : 4;
                const oneBeatMs = 60000 / bpm;
                const oneMeasureMs = oneBeatMs * beatsPerMeasure;

                let numHits = 1;
                let hitIntervalMs = oneMeasureMs;
                let noteDuration = '1n';

                if (rhythm === '4n') {
                    numHits = beatsPerMeasure;
                    hitIntervalMs = oneBeatMs;
                    noteDuration = '4n';
                } else if (rhythm === '8n') {
                    numHits = beatsPerMeasure * 2;
                    hitIntervalMs = oneBeatMs * 0.5;
                    noteDuration = '8n';
                } else if (rhythm === '2n') {
                    numHits = timeSig === '3/4' ? 1 : 2;
                    hitIntervalMs = oneBeatMs * 2;
                    noteDuration = '2n';
                }

                for (let hit = 0; hit < numHits; hit++) {
                    playChord(chord.midis, noteDuration);
                    if (rhythm === '2n' && timeSig === '3/4') {
                        await new Promise(resolve => setTimeout(resolve, oneBeatMs * 2));
                        await new Promise(resolve => setTimeout(resolve, oneBeatMs * 1));
                    } else {
                        await new Promise(resolve => setTimeout(resolve, hitIntervalMs));
                    }
                }
    ...
    playAllBtn.addEventListener('click', async () => {
        ...
        const runPlayback = async () => {
            ...
            for (let i = 0; i < progression.length; i++) {
                ...
                // Play the chord repeatedly within the measure based on the selected rhythm
                for (let hit = 0; hit < numHits; hit++) {
                    if (!isPlaying) break;
                    playChord(chord.midis, noteDuration);
